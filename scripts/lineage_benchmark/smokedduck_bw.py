...
#for qid in [14]:
    ...
    print("Avg: ", t/sample_size)
    results.append([qid, t/sample_size, tmin, tmax, args.sf, args.repeat, "SmokedDuck", args.threads, output_size, len(df), -1])

if args.save_csv:
    filename="tpch_bw_notes_"+args.notes+"_lineage_type_SmokedDuck.csv"
    print(filename)
    header = ["query", "runtime", "min", "max", "sf", "repeat", "lineage_type", "n_threads", "lineage_size", "base_size", "preprocess"]
    control = 'w'
    if args.csv_append:
        control = 'a'
