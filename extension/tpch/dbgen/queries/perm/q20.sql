with in2 as (
    ...
    SELECT l_partkey, l_suppkey, 0.5 * sum(l_quantity) as sum_q
    FROM in3_select
    GROUP BY l_partkey, l_suppkey
), in1 as (
  SELECT partsupp.rowid as partsupp_rowid, ps_suppkey, ps_partkey
  FROM partsupp
  WHERE ps_partkey IN ( select p_partkey from in2 )
    AND ps_availqty > (select sum_q from in3
  ...
  WHERE s_suppkey IN ( select ps_suppkey from in1 )
