...
def test_lineage_for_q(qid):
    ...
    '''
    con.execute("CREATE OR REPLACE VIEW lineage_{} AS SELECT sink.index AS input, out.index AS output, sink.chunk_id FROM HASH_GROUP_BY_{}_0_OUT AS out, HASH_GROUP_BY_{}_0_SINK AS sink WHERE out.value=sink.value".format(qid, qid, qid))
    start = time.time()
