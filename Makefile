...
EXTENSIONS:=${EXTENSIONS} -DUSE_LINEAGE=1
ifeq (${BUILD_ICU}, 1)
...
debug:
	mkdir -p build/debug && \
	cd build/debug && \
	cmake $(GENERATOR) $(FORCE_COLOR) ${WARNINGS_AS_ERRORS} ${DISABLE_UNITY_FLAG} ${DISABLE_SANITIZER_FLAG} ${STATIC_LIBCPP} ${EXTENSIONS} -DBUILD_PYTHON=1 -DCMAKE_BUILD_TYPE=Debug ../.. && \
	cmake --build .
