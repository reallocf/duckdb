...
DataTable::DataTable(DatabaseInstance &db, const string &schema, const string &table, vector<LogicalType> types_p,
                     unique_ptr<PersistentTableData> data, bool isLineageTable, bool lineage_query_as_table)
    : info(make_shared<DataTableInfo>(db, schema, table, isLineageTable, lineage_query_as_table)), types(move(types_p)), db(db), total_rows(0), is_root(true) {
