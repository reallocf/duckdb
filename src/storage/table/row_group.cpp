...
#ifdef LINEAGE
      if (state.parent.lineage_op && approved_tuple_count > 0) {
        auto lop = reinterpret_cast<TableScanLineage*>(state.parent.lineage_op.get());
        lop->lineage.push_back({sel.sel_data(), approved_tuple_count, this->start, state.vector_index});
      }
#endif
		}

#ifdef LINEAGE
    if (state.parent.lineage_op && count > 0) {
      auto lop = reinterpret_cast<TableScanLineage*>(state.parent.lineage_op.get());
      lop->lineage.push_back({nullptr, count, this->start, state.vector_index});
    }
#endif
