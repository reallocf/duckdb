...
#ifdef LINEAGE
  list.back()->lineage_op = lineage_op;
#endif
...
idx_t PartitionableHashTable::AddChunk(DataChunk &groups, DataChunk &payload, bool do_partition) {
  ...
  lop->radix_log.emplace_back();
    ...
    if (lop->addchunk_log.size() > 0)
      lop->radix_log.back().push_back({r, sel_vectors[r], sel_vector_sizes[r], &lop->addchunk_log.back()});
#endif
	}
...
void PartitionableHashTable::Partition() {
	...
	for (auto &unpartitioned_ht : unpartitioned_hts) {
#ifdef LINEAGE
      unpartitioned_ht->lineage_op = lineage_op;
#endif
		for (idx_t r = 0; r < partition_info.n_partitions; r++) {
      ...
      partition_hts.back()->lineage_op = lineage_op;
#endif
		}
