...
public:
	...
	virtual void persist(ClientContext &context) = 0;
};
...
public:
	...
	void persist(ClientContext &context) {
		// do this once during query initialization
        string tablename = "FILTER_L";
        string schema = "main";
        TableCatalogEntry * table = Catalog::GetCatalog(context).GetEntry<TableCatalogEntry>(context, schema, tablename);
		std::cout << tablename << " " << table->columns.size() << " " << table->ColumnExists("i") << std::endl;
		insert_chunk.Reset();
        insert_chunk.Initialize(table->GetTypes());
        insert_chunk.SetCardinality(count);
        this->debug();
        Vector row_ids;
        row_ids.SetType(LogicalType::INTEGER);
        FlatVector::SetData(row_ids, (data_ptr_t)&vec[0]);
        insert_chunk.data[0].Reference(row_ids);
		// add chunk ID as well
		// eager
        std::cout << "insert_chunk " << (vec[0]) <<" " << insert_chunk.ToString() << std::endl;
