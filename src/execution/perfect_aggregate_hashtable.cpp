...
#ifdef LINEAGE
void PerfectAggregateHashTable::Scan(idx_t &scan_position, DataChunk &result, shared_ptr<OperatorLineage> lineage_op) {
#else
...
#ifdef LINEAGE
  auto lop = reinterpret_cast<PHALineage*>(lineage_op.get());
  lop->scan_lineage.push_back(move(group_values_smrt));
#endif
}
