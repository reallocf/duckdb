...
idx_t GroupedAggregateHashTable::AddChunk(DataChunk &groups, Vector &group_hashes, DataChunk &payload) {
	...
	std::cout << addresses.ToString(groups.size()) << std::endl;
#ifdef LINEAGE
...
idx_t GroupedAggregateHashTable::Scan(idx_t &scan_position, DataChunk &result) {
	...
	std::cout << addresses.ToString(result.size()) << std::endl;
#ifdef LINEAGE
	// this maps input to groups, we can use it later to map output to input
	auto scan_lineage = make_unique<LineageDataVectorBufferArray>(move(addresses.GetBuffer()->data),  result.size());
