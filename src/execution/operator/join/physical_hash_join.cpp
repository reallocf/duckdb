    ...
    do {
      ...
      if (chunk.size() == 0) {
        ...
        if (trace_lineage && chunk.size() > 0) {
          ...
          lop->cached_output_index.push_back(lop->lineage_binary.size()-1);
        }
#endif
          ...
          lop->lineage_binary.push_back({move(state->scan_structure->lineage_left),
            move(state->scan_structure->lineage_right),
            chunk.size(), state->child_state->out_start});
          lop->output_index.push_back(lop->lineage_binary.size()-1);
				}
#endif
				return;
			}
          ...
          lop->lineage_binary.push_back({move(state->scan_structure->lineage_left),
              move(state->scan_structure->lineage_right),
              chunk.size(), state->child_state->out_start});
          lop->output_index.push_back(lop->lineage_binary.size());
			}
#endif
			return;
#endif
