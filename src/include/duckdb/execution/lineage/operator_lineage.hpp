...
struct LineageIndexStruct {
	...
	// Current col count
	idx_t col_count;
	// The number of records output from this IndexJoin so far
	idx_t output_so_far;
	...
	// vector of vector of Vectors since each cached value may have more than one value ex: when there are JOINs then AGGs
