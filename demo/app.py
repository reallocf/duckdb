...
def toggleLineage():
  ...
  toggle = req["toggle"]
  # TODO is this right?
  query(f"pragma LINEAGE={'ON' if toggle else 'OFF'}")
...
def executeSQL():
  ...
  req = request.json
  sqls = req["sqls"]
  res = {"res": {sql["name"]: jsonifyDuckData(query(sql["value"]).fetchall()) for sql in sqls}}
  return res

@app.route("/initial", methods=["POST"])
def getInitialData():
  global expectedCauses, expectedStates, expectedYears
  causeData = jsonifyDuckData(query("SELECT STAT_CAUSE_DESCR, COUNT(*) FROM fires GROUP BY STAT_CAUSE_DESCR").fetchall())
  expectedCauses = [d["x"] for d in causeData]
  stateData = jsonifyDuckData(query("SELECT STATE, COUNT(*) FROM fires GROUP BY STATE").fetchall())
  expectedStates = [d["x"] for d in stateData]
  yearData = jsonifyDuckData(query("SELECT FIRE_YEAR, COUNT(*) FROM fires GROUP BY FIRE_YEAR").fetchall())
